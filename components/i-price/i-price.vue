<template>
	<view class="price" style="font-weight: 600;" :style="{ color: status === 'del' ? delColor : priceColor }">
		<view class="price-icon" :class="[icon ? 'price-icon-' + icon : '', status ? 'price-icon-' + status : '']" :style="{ fontSize: symbolSize + 'rpx' }">{{ symbol }}</view>

		<view class="price-count" :class="[status ? 'price-count-' + status : '']" :style="{ fontSize: countSize + 'rpx' }">
