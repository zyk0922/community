<template>
	<view class="content">
		<view class="from">
			...
			<view class="textarea"><textarea class="textareaa" v-model="content" placeholder="请输入报修问题,以便维修人员尽快为您解决"
					placeholder-style="font-size:24rpx" /></view>
			<view class="camera">
				<view class="camera-btn" @touchstart="Videopicture()"><i class="iconfont icon-xiangji"></i></view>
				<view class="camera-img" v-for="(item,index) in imgList" :key="index">
					<i class="iconfont icon-chahao" @touchstart="delimg(index)"></i>
					<image class="img" :src="$config.imgUrl+item.name" mode=""></image>
				</view>
			</view>
			<view class="btn" @touchstart="addform()">提交</view>
		</view>
	</view>
</template>
	...
	export default {
		...
		methods: {
			...
			addform() {
				...
				this.$http.post('/api/user/questions/add', form).then(res => {
					uni.redirectTo({
			...
			// 获取报修类型
			gettype() {
				this.$http.get('/api/user/questiontype/list', {}).then(res => {
